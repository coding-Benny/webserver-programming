...
<jsp:useBean id="login" class="jspbook.addrbook.Login" />
...
<script type="text/javascript">
	...
	function view(ab_id) {
		document.location.href="addrbook_control.jsp?action=view&ab_id="+ab_id;
	}
</script>
...
<table border="1">
  <tr><th>번호</th><th>이 름</th><th>전화번호</th><th>생 일</th><th>주 소</th><th>회 사</th><th>메 모</th></tr>
	<%
		for(AddrBook  ab : (ArrayList<AddrBook>)datas) {
	%>
  <tr><td><a href="javascript:check(<%=ab.getAb_id()%>)"><%=ab.getAb_id() %></a></td>
		<td><a href="javascript:view(<%=ab.getAb_id()%>)"><%=ab.getAb_name() %></a></td>
		<td><%=ab.getAb_tel() %></td>
