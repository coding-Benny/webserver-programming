 ...
 * @author benny(sjeonghyeonz@gmail.com)
 */
public class AddrBean {
	 ...
	 * String jdbc_url = "jdbc.oracle:thin:@220.60.14.7:1521";
	 */
	...
	public boolean updateDB(AddrBook addrbook) {
		...
		String sql = "update addrbook set ab_name=?, ab_email=?, ab_birth=?, ab_tel=?, ab_addr=?, ab_comdept=?, ab_memo=? where ab_id=?";
		...
		try {
			...
			pstmt.setString(5, addrbook.getAb_addr());
	...
	public boolean insertDB(AddrBook addrbook) {
		connect();
		// sql 문자열, gb_id는 자동 등록 값으로 입력하지 않는다.
		
		String sql = "insert into addrbook(ab_name, ab_email, ab_birth, ab_tel, ab_addr, ab_comdept, ab_memo) values(?,?,?,?,?,?,?)";
